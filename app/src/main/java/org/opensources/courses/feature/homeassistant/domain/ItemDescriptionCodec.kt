 ...
 * `1,5 kg`, `1.5 kg` in English) for lists that support descriptions. A quantity of 1 without unit
 * clears the description. A description that does not start with a number is read as quantity 1.
 * Both decimal separators are read, so phones in different languages share the same lists.
 */
object ItemDescriptionCodec {
    private val PATTERN = Regex("^\\s*(\\d+(?:[.,]\\d+)?)\\s*(.*?)\\s*$")

    /** Written with the decimal separator of [locale], the language of the app. */
    fun encode(
        quantity: Double,
        unit: String?,
        locale: Locale,
    ): String? = if (quantity == 1.0 && unit.isNullOrBlank()) null else QuantityFormatter.format(quantity, unit, locale)
